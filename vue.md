...
View是视图，也就是DOM，视图部分就是HTML部分。Model是数据模型，也就是数据（vue里的data，或者vuex里的数据），可以在Model中定义数据修改和操作的业务逻辑。ViewModel用于监听模型数据（监听data的改变、控制视图行为、处理用户交互），简单来讲就是一个同步View和Model的对象，连接Model和View。
...
1. 降低系统耦合度
2. 方便调试，每个组件职责单一，开发出现问题可以迅速定位到某一个组件
...
2. 前后端一套框架，不需要，通俗讲就是前端代码也可以在后端跑；
...
v-if比v-for的优先级更高，如果在同一个元素上同时使用了它俩，v-if将不能使用v-for中的变量，导致出错。
...
双向绑定：更新数据，视图也会随之更新，而视图变化，数据也会更新。实现方式是数据绑定+事件回调，对应的语法糖v-model，这样可以省掉大量的处理代码，可读性更好。通常v-model会用在表单上，也可以用在组件上，当用在组件上的时候，需要在组件内部处理modelValue和update:modelValue。v-model实际上会被转换成:value和@input（表单上）或:modelValue和@update:modelValue（组件上）。
...
必要性：使用虚拟DOM+diff算法，可以减少DOM操作，提升更行性能；
...
vue3引入的patch优化：patchFlags（在compile阶段，将使用了动态绑定的地方进行标记，这样在runtime阶段使用patch的时候，可快速找出要patch的地方，靶向更新，性能高更）、block。
