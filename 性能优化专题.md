...
- dns-prefetch，在正式请求跨域资源之前，就对域名进行dns解析，这样在正式请求资源时，可以省掉dns解析（得到ip地址）的步骤，降低一些延迟，这对有大量第三方资源的页面性能提升非常明显。
  ...
  ```html
  // 每当站点引用跨域域上的资源时，都应在<head>元素中放置 dns-prefetch 提示
...
3. 如果使用动画，尽量把动画放在absolute或fixed的元素上，因为脱离文档流的元素不会被reflow；
...
4. 实在避不开TCP的满启动，就将首屏所需的数据压到14KB以内，便于在慢启动的最初14KB就满足首屏渲染所需的数据；
